...
cumulative(const std::size_t reportStepNum) const
{
    ...
    for (const auto& gname : this->schedule_.groupNames()) {
        std::vector<Scalar> values(WellCumDataType::numWCValues, 0.0);
        std::vector<std::string> names(WellCumDataType::numWCNames, "");

        names[Ix::WellName] = gname;

        if (gname == "FIELD") {
            values[Ix::OilProd]         = st.get("FOPT", 0.0);
            values[Ix::WaterProd]       = st.get("FWPT", 0.0);
            values[Ix::GasProd]         = st.get("FGPT", 0.0);
            values[Ix::FluidResVolProd] = st.get("FVPT", 0.0);
            values[Ix::OilInj]          = st.get("FOIT", 0.0);
            values[Ix::WaterInj]        = st.get("FWIT", 0.0);
            values[Ix::GasInj]          = st.get("FGIT", 0.0);
            values[Ix::FluidResVolInj]  = st.get("FVIT", 0.0);
        }
        else {
            values[Ix::OilProd]         = st.get_group_var(gname, "GOPT", 0.0);
            values[Ix::WaterProd]       = st.get_group_var(gname, "GWPT", 0.0);
            values[Ix::GasProd]         = st.get_group_var(gname, "GGPT", 0.0);
            values[Ix::FluidResVolProd] = st.get_group_var(gname, "GVPT", 0.0);
            values[Ix::OilInj]          = st.get_group_var(gname, "GOIT", 0.0);
            values[Ix::WaterInj]        = st.get_group_var(gname, "GWIT", 0.0);
            values[Ix::GasInj]          = st.get_group_var(gname, "GGIT", 0.0);
            values[Ix::FluidResVolInj]  = st.get_group_var(gname, "GVIT", 0.0);
        }
...
injection(const std::size_t reportStepNum) const
{
    ...
    for (const auto& gname : this->schedule_.groupNames()) {
        std::vector<Scalar> values(WellInjDataType::numWIValues, 0.0);
        std::vector<std::string> names(WellInjDataType::numWINames, "");

        names[Ix::WellName] = gname;

        if (names[0] == "FIELD") {
            values[Ix::OilRate]     = st.get("FOIR", 0.0);
            values[Ix::WaterRate]   = st.get("FWIR", 0.0);
            values[Ix::GasRate]     = st.get("FGIR", 0.0);
            values[Ix::FluidResVol] = st.get("FVIR", 0.0);
        }
        else {
            values[Ix::OilRate]     = st.get_group_var(gname, "GOIR", 0.0);
            values[Ix::WaterRate]   = st.get_group_var(gname, "GWIR", 0.0);
            values[Ix::GasRate]     = st.get_group_var(gname, "GGIR", 0.0);
            values[Ix::FluidResVol] = st.get_group_var(gname, "GVIR", 0.0);
        }
...
production(const std::size_t reportStepNum) const
{
    ...
    for (const auto& gname : this->schedule_.groupNames()) {
        std::vector<Scalar> values(WellProdDataType::numWPValues, 0.0);
        std::vector<std::string> names(WellProdDataType::numWPNames, "");

        names[0] = gname;

        if (names[0] == "FIELD") {
            values[Ix::OilRate]     = st.get("FOPR", 0.0);
            values[Ix::WaterRate]   = st.get("FWPR", 0.0);
            values[Ix::GasRate]     = st.get("FGPR", 0.0);
            values[Ix::FluidResVol] = st.get("FVPR", 0.0);
            values[Ix::WaterCut]    = st.get("FWCT", 0.0);
            values[Ix::GasOilRatio] = st.get("FGOR", 0.0);
        }
        else {
            values[Ix::OilRate]     = st.get_group_var(gname, "GOPR", 0.0);
            values[Ix::WaterRate]   = st.get_group_var(gname, "GWPR", 0.0);
            values[Ix::GasRate]     = st.get_group_var(gname, "GGPR", 0.0);
            values[Ix::FluidResVol] = st.get_group_var(gname, "GVPR", 0.0);
            values[Ix::WaterCut]    = st.get_group_var(gname, "GWCT", 0.0);
            values[Ix::GasOilRatio] = st.get_group_var(gname, "GGOR", 0.0);
        }
