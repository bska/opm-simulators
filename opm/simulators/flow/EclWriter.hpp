...
#pragma omp parallel for
                     ...
                     const bool            isSubStep)
    {
        ...
        if (! isSubStep || Parameters::get<TypeTag, Parameters::EnableWriteAllSolutions>()) {
            if (const auto rstep = timer.reportStepNum();
                (rstep > 0) && this->collectOnIORank_.isIORank())
            {
                outputModule_->outputFipAndResvLog(inplace_, rstep,
                ...
                outputModule_->outputTimeStamp("WELLS", timer.simulationTimeElapsed(),
                                               rstep, timer.currentDateTime());
                              ...
                              const int  reportStepNum)
    {
        ...
        this->outputModule_->
            allocBuffers(num_interior, reportStepNum,
                         isSubStep && !Parameters::get<TypeTag, Parameters::EnableWriteAllSolutions>(),
                         log, /*isRestart*/ false);
